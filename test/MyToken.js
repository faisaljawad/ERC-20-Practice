...
contract('MyToken', function (accounts) {
  ...
  it('initializes the contract with the correct values', function () {
    return MyToken.deployed()
      ...
      .then(function (name) {
        assert.equal(name, 'Faisal Token', 'has a correct name')
        return tokenInstance.symbol()
      })
      .then(function (symbol) {
        assert.equal(symbol, 'DAPP', 'has a correct symbol')
        return tokenInstance.standard()
      })
      .then(function (standard) {
        assert.equal(standard, 'DApp Token v1.0', 'has a correct standard')
      })
  })
  ...
  it('approves tokens for delegated transfer', function() {
    return MyToken.deployed().then(function(instance) {
      ...
      assert.equal(allowance.toNumber(), 100, 'stores the allowance for delegrated transfer')
    });
  })
